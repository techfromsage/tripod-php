...
usage() { echo "Usage: $0 [-r <repset-number>] [-n <number-of-nodes>] [-p </base/path/to/database>]" 1>&2; exit 0; }

while getopts r:n:p: opt; do
	case "${opt}" in
		r)
			: $((REPSET_NUMBER=${OPTARG}-1))
			REPSET_NAME="rs"
...
while [ $COUNTER -lt $NODE_COUNT ]; do
	...
	mongod --port $MONGO_PORT --dbpath $CURR_PATH --replSet $REPSET_NAME --smallfiles --oplogSize 128
done
